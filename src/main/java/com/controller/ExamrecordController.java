...
//                            examrecordEntity.setExamrecordUuidNumber(data.get(0));                    //测试编号 要改的
//                            examrecordEntity.setYonghuId(Integer.valueOf(data.get(0)));   //测试用户 要改的
...
//                            examrecordEntity.setCreateTime(date);//时间
                        ...
                        if(examrecordEntities_examrecordUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [测试编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R deleteExamrecord(@RequestBody Integer[] ids){
        logger.debug("delete:,,Controller:{},,ids:{}",this.getClass().getName(),ids.toString());
        List<ExamrecordEntity> examrecordE = examrecordService.selectBatchIds(Arrays.asList(ids));//测试记录表
